...
**中文概述：** 这是我的学校项目索引，集中展示课程最终项目、研究成果和可复现性说明。小组作品保留团队归属；课程提交、课程数据和 EVRP 原始报告采用私有保存。私有链接需要仓库访问权限。
...
| [CIFAR-100 Classification Experiments](https://github.com/yongjiang-niuniu/cifar-100) | ResNet and Vision Transformer classification notebooks, repaired ResNet-50 workflow, preserved original experiments, and offline checks | Python, PyTorch, torchvision, Jupyter | Public; ResNet-50 offline execution checked on generated images; historical metrics are qualified separately |
...
This public repository is the entry point for ten project repositories. Its `projects/` pages explain selected private work without publishing the underlying reports or teaching materials. The [documentation guide](docs/README.md) links the maintenance policy and explains how to read the collection.

The separate [private academic archive](https://github.com/yongjiang-niuniu/academic-project-archive) records all course coverage, submission sources, verified attachments, and current project revisions. Together these are 12 repositories: ten projects and two indexes.
